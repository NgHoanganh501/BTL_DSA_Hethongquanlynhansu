...
void DayOffInput(DayOff *doff){
    ...
    clear_input_buffer();
    doff->AbsenceLetter = letterToString(absenceletteryesno);
    ...
    clear_input_buffer();
    doff->letterstatus = astatusToString(absenceletterstatus);
}
...
void EducationInput(Education *e){
    ...
    clear_input_buffer();
    e->coursestatus = cstatusToString(coursestatus);
}
...
void LayOffInput(LayOff *lo){
    ...
    scanf("%d", &rson);
    clear_input_buffer();
    lo->Reason = layoffreasonToString(rson);
}
